...
| `WATCH_LIBRARY`                            | false                              | Watch the library instead of walking the folders. This does not disable normal scans from `SCAN_INTERVAL`. Respects `SCAN_ON_STARTUP`                                                                                                                                                                                                                                                                                                                                 |
| `WATCH_POLLING_INTERVAL`                   | 0                                  | Whether to use fs.watchFile (backed by polling), or fs.watch. If polling leads to high CPU utilization, consider setting this to false. It is typically necessary to set this to true to successfully watch files over a network, and it may be necessary to successfully watch files in other non-standard situations (Docker). A positive value will enable this, and the value will be used as the polling interval - only works when `SCAN_ON_STARTUP` is enabled |
