...
export async function addAutoGeneratedFlag(file: string) {
  logger.log("Adding missing autoGenerated flags in " + file);
  
  const libraryDir = getConfig().LIBRARY_PATH + "\\library";
  logger.log("library directory is " + libraryDir);
  ...
  await transformFile(file, (line) => {
    ...
    if (item.autoGenerated == undefined) {
      logger.log(item.path);
      if(item.path.startsWith(libraryDir)) {
        ...
        //TODO: Autodetect auto-generated based on library path
        return JSON.stringify(item);
